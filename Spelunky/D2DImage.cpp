...
void D2DImage::RenderMatrix(ID2D1HwndRenderTarget *renderTarget, float destX, float destY, float sourX, float sourY, 
		float sourWidth, float sourHeight, const D2D1::Matrix3x2F &mat, float alpha)
{
	Assert(_bitmap);
	D2D1::Matrix3x2F matTranslation;
	matTranslation = D2D1::Matrix3x2F::Translation(destX, destY);
	renderTarget->SetTransform(mat * matTranslation );
